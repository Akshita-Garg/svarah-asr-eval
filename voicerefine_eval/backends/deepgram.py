"""Deepgram pre-recorded transcription (Nova family)."""
...
def _bool_param(value: bool) -> str:
    # Deepgram reads query parameters as strings; be explicit rather than
    # relying on requests' bool coercion.
...
class DeepgramBackend(ASRBackend):
    def __init__(self, cfg: BackendConfig):
        super().__init__(cfg)
        self.model = self.settings.get("model", "nova-3")
        self.language = self.settings.get("language", "en")
        self.punctuate = bool(self.settings.get("punctuate", True))
        # smart_format rewrites numbers, currency and dates. Left off so the
        # Whisper normalizer is the only formatting pass applied, matching how
        # every other backend in this evaluation is scored.
        self.smart_format = bool(self.settings.get("smart_format", False))
    ...
    def start(self) -> None:
        ...
        if not key:
            raise BackendUnavailableError(
                "DEEPGRAM_API_KEY is not set (put it in .env). Skipping cloud backend."
            )
        self._api_key = key
    ...
    def _extract_transcript(self, body: Any) -> str:
        """Pull the transcript out of Deepgram's nested response shape.
        ...
        try:
            ...
            raise TranscriptionError(
                f"Unexpected Deepgram response shape: {str(body)[:200]}",
                category="bad_response",
            ) from error
        if not isinstance(transcript, str):
            raise TranscriptionError(
                f"Deepgram transcript is not a string: {str(body)[:200]}",
                category="bad_response",
            )
    ...
    def transcribe(self, audio_path: Path) -> str:
        ...
        if isinstance(last_error, TranscriptionError):
            raise last_error
        raise TranscriptionError(
            str(last_error) if last_error else "Deepgram transcription failed",
